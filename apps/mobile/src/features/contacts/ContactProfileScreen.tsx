...
export function ContactProfileScreen({ route, navigation }: any) {
  ...
  useFocusEffect(useCallback(() => { loadContact(); }, [contactId]));
  ...
  const handleOpenChat = async () => {
    try {
      // If we don't have conversationId, fetch from server
      let conversationId: string | undefined = route.params.conversationId;

      if (!conversationId) {
        const { data } = await chatApi.getConversations();
        const conv = data.find((c: any) => c.otherUser.id === otherUser.id);
        if (!conv) {
      ...
      if (!conversationId) {
        showToast('error', 'Failed to find conversation');
        return;
      }
        ...
        { text: 'Cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              showToast('error', err.response?.data?.error || 'Failed to block contact');
            } finally {
              setSaving(false);
            }
          },
        ...
        { text: 'Cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              useConversationsStore.getState().addHiddenConversation(route.params.conversationId!);
              showToast('success', 'Conversation deleted');
            } catch (err: any) {
              showToast('error', err.response?.data?.error || 'Failed to delete conversation');
            } finally {
              setSaving(false);
            }
          },
      ...
      [
        { text: 'Cancel' },
        {
          ...
          onPress: async () => {
            try {
              setSaving(true);
              await chatApi.deleteConversation(route.params.conversationId!, 'everyone');
              useConversationsStore.getState().addHiddenConversation(route.params.conversationId!);
    ...
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.headerSection}>
        ...
        <View style={styles.presenceRow}>
          <View
            style={[styles.onlineDot, { backgroundColor: isOnline ? colors.primary : colors.gray400 }]}
          />
        ...
        >
          <Text style={styles.relationshipText}>
            {relationshipEmoji} {relationshipType.charAt(0).toUpperCase() + relationshipType.slice(1)}
          </Text>
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.section}>
        ...
        <TouchableOpacity
          style={styles.row}
          onPress={() => setNicknameModal({ visible: true, field: 'nickname', value: contact.nickname || '' })}
          disabled={saving}
        >
          <View style={styles.rowContent}>
            <Text style={styles.rowLabel}>Contact's nickname</Text>
            <Text style={styles.rowValue}>
              {contact.nickname || 'Add nickname'}
            </Text>
          </View>
        </TouchableOpacity>
        ...
        <TouchableOpacity
          style={styles.row}
          onPress={() => setNicknameModal({ visible: true, field: 'myNickname', value: contact.myNickname || '' })}
          disabled={saving}
        >
          <View style={styles.rowContent}>
            <Text style={styles.rowLabel}>My nickname</Text>
            <Text style={styles.rowValue}>
              {contact.myNickname || 'Add nickname'}
            </Text>
          </View>
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.section}>
        ...
        <TouchableOpacity
          ...
          disabled={saving}
        >
          <Text style={[styles.actionButtonText, { color: colors.dark }]}>Remove contact</Text>
        </TouchableOpacity>
        ...
        >
          <View style={[styles.modalBox, { backgroundColor: colors.bg }]}>
            <Text style={[styles.modalTitle, { color: colors.dark }]}>
              {nicknameModal.field === 'nickname' ? "Set nickname" : "Set my nickname"}
            </Text>
            ...
            <TextInput
              style={[styles.modalInput, { color: colors.dark, borderColor: colors.border, backgroundColor: colors.bgSecondary }]}
              value={nicknameModal.value}
              onChangeText={(text) => setNicknameModal((s) => ({ ...s, value: text }))}
              autoFocus
